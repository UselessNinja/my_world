...
int draw_2d_map(sfRenderWindow *window, sfVector2f **map_2d)
{
	...
	for (count_y = 0; count_y < MAP_Y; count_y++)
		for (count_x = 0; count_x < MAP_X; count_x++) {
			if (count_x == MAP_X - 1 && count_y == MAP_Y - 1) {
				break;
			} else if (count_x == MAP_X - 1) {
				vertex_y(window, map_2d, count_x, count_y);
			} else if (count_y == MAP_Y - 1) {
				vertex_x(window, map_2d, count_x, count_y);
			} else {
				vertex_x(window, map_2d, count_x, count_y);
				vertex_y(window, map_2d, count_x, count_y);
			}
